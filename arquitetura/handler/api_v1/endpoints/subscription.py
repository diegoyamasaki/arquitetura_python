...
from arquitetura.infra.schema.subscription import SubscriptionSchemaCreate, \
    SubscriptionSchemaUpdate
...
def subscriptions(
        application: SubscriptipnApplication
        = Depends(SubscriptipnApplication),
        db: Session = Depends(get_db),
        repository: SubscriptionRepository
        = Depends(SubscriptionRepository)):
...
@subscription_route.put(
    '/{subscription_id}',
    status_code=status.HTTP_201_CREATED)
def put_subscription(
        subscription_id: str,
        data: SubscriptionSchemaUpdate,
        application: SubscriptipnApplication
        = Depends(SubscriptipnApplication),
        db: Session = Depends(get_db),
        repository: SubscriptionRepository
...
def delete_subscription(
    ...
    return application.delete(subscription_id,db, repository)
