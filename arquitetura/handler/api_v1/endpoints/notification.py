...
def notifications(application: NotificationApplication = Depends(NotificationApplication)):
    return application.get_all()


@notification_route.post('/', status_code=status.HTTP_201_CREATED)
def post_notification(data: NotificationSchemaBase, application: NotificationApplication = Depends(NotificationApplication)):
    user = application.create(data)
    return user


@notification_route.delete('/{notification_id}', status_code=status.HTTP_201_CREATED)
def delete_notification(notification_id: str, application: NotificationApplication = Depends(NotificationApplication)):
